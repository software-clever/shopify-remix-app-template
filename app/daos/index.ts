...
export const initShopifyDataManagers = (graphqlClient: AdminGraphqlClient) => {
    const queryExecutor = new ShopifyQueryExecutor(graphqlClient, new FileContentReader());
...
export const dataManagers = {
    get shopify(): ShopifyDataManagers {
        if (!shopifyDataManagers) throw new Error("ShopifyDataManagers not initialized!");
